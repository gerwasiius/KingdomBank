...
namespace Identity.Worker.Repositories;
...
public sealed class KeyRotationEfRepository : IKeyRotationRepository
{
    ...
    public async Task InsertAsync(Models.KeyDescriptor d, string status, CancellationToken ct = default)
    {
        var row = new KeyRotationEntity
        {
            Kid = d.Kid,
            Alg = d.Algorithm == Models.KeyAlgorithm.ES256 ? "ES256" : "RS256",
            Status = status,
